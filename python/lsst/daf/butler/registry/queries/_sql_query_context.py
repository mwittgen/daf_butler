...
import sqlalchemy
from lsst.daf.relation import Leaf, Relation, TransferVisitor, iteration

from ...core import ButlerSqlEngine, ColumnTag, TimespanDatabaseRepresentation
...
class SqlQueryContext(QueryContext):
    ...
    def __init__(
        ...
        self._local_engine = iteration.Engine[ColumnTag]("local")
    ...
    def fetch_iterable(self, relation: Relation[ColumnTag]) -> iteration.RowIterable[ColumnTag]:
        # Docstring inherited.
        if relation.engine is not self._local_engine:
            iteration_relation = self._sql_to_iteration(relation)
        else:
            visitor = TransferVisitor[ColumnTag](
                {(self._sql_engine, self._local_engine): self._sql_to_iteration}
            )
            iteration_relation = relation.visit(visitor)
        return self._local_engine.execute(iteration_relation)
    ...
    def _sql_to_iteration(self, source: Relation[ColumnTag]) -> Relation[ColumnTag]:
        ...
        """
        sql_executable = self._sql_engine.to_executable(source)
        rows = _SqlRowIterable(self._db, sql_executable, _SqlRowTransformer(source.columns, self._sql_engine))
        leaf = Leaf(
            str(source), self._local_engine, source.columns, unique_keys=source.unique_keys, parameters={}
        )
        self._local_engine.leaf_cache[leaf] = rows
        return leaf
...
class _SqlRowTransformer:
    ...
    def __init__(self, columns: Iterable[ColumnTag], sql_engine: ButlerSqlEngine):
        self._scalar_columns = set(columns)
        self._timespan_columns: dict[ColumnTag, type[TimespanDatabaseRepresentation]] = {}
        self._timespan_columns.update({tag: sql_engine.timespan_cls for tag in columns if tag.is_timespan})
        self._scalar_columns.difference_update(self._timespan_columns.keys())

    __slots__ = ("_scalar_columns", "_timespan_columns")
